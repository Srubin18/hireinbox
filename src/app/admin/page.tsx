...
// Stat Card Component
const StatCard = ({ icon, iconBg, value, label, sublabel, trend, gradient, onClick }: {
  icon: string;
  iconBg: string;
  value: string;
  label: string;
...
const ActivityItem = ({ activity }: { activity: ActivityLog }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 0', borderBottom: '1px solid #f1f5f9' }}>
      <div style={{ width: 36, height: 36, borderRadius: 8, background: bgColors[activity.type] || '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        <span style={{ fontSize: '1rem' }}>{icons[activity.type] || '📋'}</span>
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
