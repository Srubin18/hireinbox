...
function replaceTemplateVariables(template: string, variables: TemplateVariables): string {
  ...
  const replacements: Record<string, string> = {
    ...
    '{{feedbackLink}}': variables.feedbackLink || '',
...
The {{companyName}} Hiring Team`
  ...
  candidate_feedback: {
    subject: 'Your Application Feedback - {{roleTitle}} at {{companyName}}',
    bodyHtml: `
      <p style="margin: 0 0 24px 0; font-size: 16px; color: ${COLORS.primary};">
        Hi {{candidateFirstName}},
      </p>
      ...
      <p style="margin: 0 0 20px 0; font-size: 16px; color: ${COLORS.secondary};">
        We believe every candidate deserves to understand where they stand. We have prepared personalized feedback on your application based on the role requirements.
      </p>

      <div style="background-color: #F0F9FF; border: 1px solid #BFDBFE; border-radius: 12px; padding: 24px; margin: 0 0 24px 0;">
        <p style="margin: 0 0 8px 0; font-size: 14px; color: ${COLORS.secondary}; text-transform: uppercase; letter-spacing: 0.5px;">
          Your Personalized Feedback
        </p>
        <p style="margin: 0 0 16px 0; font-size: 15px; color: ${COLORS.primary};">
          See what matched, areas for improvement, and actionable suggestions to strengthen future applications.
        </p>
      ...
      <p style="margin: 0 0 20px 0; font-size: 15px; color: ${COLORS.secondary};">
        Your feedback includes:
      </p>
      <ul style="margin: 0 0 24px 0; padding-left: 20px;">
        <li style="margin-bottom: 8px; font-size: 15px; color: ${COLORS.secondary};">Strengths we identified in your application</li>
        <li style="margin-bottom: 8px; font-size: 15px; color: ${COLORS.secondary};">Areas where the requirements were not fully met</li>
        <li style="margin-bottom: 8px; font-size: 15px; color: ${COLORS.secondary};">Specific suggestions to improve your CV</li>
        <li style="margin-bottom: 8px; font-size: 15px; color: ${COLORS.secondary};">Alternative roles that may suit your experience</li>
      </ul>

      <p style="margin: 0 0 20px 0; font-size: 14px; color: ${COLORS.muted};">
        This feedback link is unique to you and will remain accessible for 30 days. If you believe something was missed, you can request a human review directly from the feedback page.
      </p>
...
VIEW YOUR FEEDBACK: {{feedbackLink}}

Your feedback includes:
- Strengths we identified in your application
- Areas where the requirements were not fully met
- Specific suggestions to improve your CV
- Alternative roles that may suit your experience
 ...
 * Trigger: MANUAL or AUTOMATIC - after screening decision
 * Purpose: Provide transparency, comply with Colorado law (2026), build goodwill
...
export async function sendCandidateFeedbackEmail(
  ...
  return sendTemplatedEmail('candidate_feedback', to, {
    candidateName,
    roleTitle,
    companyName: branding?.name || 'HireInbox',
    feedbackLink,
...
/**
 * Generate a feedback link URL for candidates
 */
export function generateFeedbackLinkUrl(token: string, baseUrl?: string): string {
  const base = baseUrl || process.env.NEXT_PUBLIC_APP_URL || 'https://hireinbox.co.za';
