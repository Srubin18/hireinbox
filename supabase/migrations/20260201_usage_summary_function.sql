...
BEGIN
  ...
  SELECT
    pbe.event_month,
    COUNT(*) FILTER (WHERE pbe.event_type = 'talent_search') AS talent_searches,
    COALESCE(SUM((pbe.metadata->>'candidates_found')::int) FILTER (WHERE pbe.event_type = 'talent_search'), 0) AS candidates_found,
    COUNT(*) FILTER (WHERE pbe.event_type = 'role_created') AS roles_created
